CREATE TYPE "public"."chain_intent_type" AS ENUM('create_task', 'submit_receipt', 'approve_task', 'reject_task', 'cancel_task', 'withdraw');--> statement-breakpoint
...
CREATE TYPE "public"."verification_check_status" AS ENUM('passed', 'failed', 'timed_out', 'error', 'skipped');--> statement-breakpoint
...
CREATE TYPE "public"."task_chain_status" AS ENUM('unknown', 'open', 'receipt_submitted', 'approved', 'rejected', 'cancelled', 'expired');--> statement-breakpoint
CREATE TYPE "public"."task_offchain_status" AS ENUM('draft', 'awaiting_chain', 'active', 'archived');--> statement-breakpoint
...
CREATE TABLE "cli_tokens" (
	...
	CONSTRAINT "cli_tokens_digest_no_plaintext" CHECK (length("cli_tokens"."token_digest") >= 43)
);
--> statement-breakpoint
...
CREATE TABLE "tasks" (
	...
	"chain_status" "task_chain_status" DEFAULT 'unknown' NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
...
CREATE TABLE "verification_checks" (
	...
	CONSTRAINT "verification_checks_key_format" CHECK ("verification_checks"."check_key" ~ '^[a-z0-9]+(?:[-_][a-z0-9]+)*$'),
...
ALTER TABLE "chain_transactions" ADD CONSTRAINT "chain_transactions_task_id_tasks_id_fk" FOREIGN KEY ("task_id") REFERENCES "public"."tasks"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "chain_transactions" ADD CONSTRAINT "chain_transactions_replaced_by_transaction_id_chain_transactions_id_fk" FOREIGN KEY ("replaced_by_transaction_id") REFERENCES "public"."chain_transactions"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "evidence_bundles" ADD CONSTRAINT "evidence_bundles_task_id_tasks_id_fk" FOREIGN KEY ("task_id") REFERENCES "public"."tasks"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "evidence_bundles" ADD CONSTRAINT "evidence_bundles_submitted_by_token_id_cli_tokens_id_fk" FOREIGN KEY ("submitted_by_token_id") REFERENCES "public"."cli_tokens"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
