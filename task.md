...
- [x] Return only allowed fields. `receiptDto` in `apps/web/src/server/receipt-handlers.ts` builds the wire response as a literal object (never a spread), and a test asserts the exact key set of both the receipt object and its nested `checks`/`verifierAttestation` objects, proving persisted-but-unlisted fields (full check stdout/stderr previews, internal task descriptions) never leak.
- [x] Provide safe bundle download. The receipt includes `evidenceBundlePublicId`, a reference to the existing public `GET /api/v1/evidence/[evidenceId]` bundle-detail route from 4.6 (the actual safe-download/detail mechanism); the receipt itself only carries terse check summaries, not full output.
- [x] Include chain/explorer metadata and integrity status. Response includes `chainId`, `contractAddress`, `chainTaskId`, `submissionTransactionHash`, a derived `explorerTransactionUrl` (`NEXT_PUBLIC_MONAD_EXPLORER_URL` + `/tx/`), the full `verifierAttestation` (verifier address, signature, typed-data digest, expiry), and `integrityStatus` computed by independently recomputing the EIP-712 digest and recovering the signer against the deployment's configured verifier address (`computeIntegrityStatus` in `receipt-attestation.ts`), never trusting a client- or DB-declared status.
- [x] Add caching without serving stale pending lifecycle state incorrectly. Both read routes only ever return a receipt once its `submit_receipt` chain transaction is `confirmed` (a genuinely terminal onchain fact); any task without a confirmed receipt yet (no intent, pending intent, wallet-rejected, reverted) uniformly 404s with `RECEIPT_NOT_FOUND` under the existing `no-store` default. Only the confirmed, publicly-cacheable response gets a non-default `cache-control: public, max-age=60, stale-while-revalidate=30`; the member-authenticated read stays `no-store`. This avoids ever caching a non-final state as if it were final.

Routes: `POST /api/v1/tasks/[taskId]/receipt-intent` (assignee-only wallet-intent creation, mirrors `chain-intent`), `POST /api/v1/tasks/[taskId]/receipt-transactions` (wallet outcome, mirrors `chain-transactions`), `POST /api/v1/chain/reconcile-receipt/[transactionId]` (separate from the existing `chain/reconcile/[transactionId]` because it dispatches through a distinct `submit_receipt`-specific store/reconciler rather than making the generic dispatcher intent-aware), `GET /api/v1/tasks/[taskId]/receipt` (member-authenticated, cookie session, mirrors the existing `GET /api/v1/tasks/[taskId]` membership-scoped shape), `GET /api/v1/receipt/[receiptId]` (public, no auth, mirrors the `evidence/[evidenceId]` precedent — `receiptId` is the task's own opaque public ID; no separate receipt identifier is minted since a task has at most one confirmed receipt).
...
All pass: shared 16/16, evidence 36/36, db 74/74 (1 pre-existing guarded Postgres integration test skipped), cli 22/22, web 97/97 (15 new: 4 receipt-attestation, 1 receipt-runtime, 10 receipt-handlers), contracts 32/32, production build succeeds with the five new routes registered as dynamic Node routes.

Explicitly out of scope, left for milestone 4.8 per its own listed checklist: full onchain event-decoder/indexer generality (this milestone's `reconcileReceiptSubmission` decodes and verifies both `ReceiptSubmitted` and `VerifierAttestationConsumed` for the one transaction being reconciled, mirroring `reconcileTaskCreation`'s scope exactly, but persists only `ReceiptSubmitted` as an indexed `contract_events` row — `VerifierAttestationConsumed` is checked but not separately indexed); admin/debug event visibility; general reorg handling beyond the existing per-transaction reconciliation retry path. Approve/reject/cancel/withdraw chain-intent flows (their own `chainIntentType` values already existed in the schema) are not built here; the receipt's `payoutStatus`-equivalent concept from the frozen `Receipt`/`ReceiptSchema` domain type was intentionally not adopted verbatim for the wire DTO — see the work-log entry below for why.
...
## 2026-07-18 — Claude Code/coordinator — 4.7 (complete)
- Branch/worktree: `worktree-agent-a9fb3baf264e58c3b` (dedicated agent worktree of `main`)
- Summary: Completed the Public receipt API vertical slice per ADR-004. Added a `receipt_attestations` table (verifier signature, EIP-712 digest, attestation expiry, bound evidence bundle) plus two catch-up compound unique constraints (`chain_transactions_id_task_unique`, `evidence_id_task_unique`) needed to bind it safely; this migration also incidentally catches up `tasks.canonical_json`/`target_branch`/`base_commit`, `chain_transactions.response_safe_json`/`response_status`, and `tasks_project_hash_unique`, which were already present in `schema.ts` from milestone 4.5 but had never actually been captured in a generated SQL migration (a pre-existing drift discovered while running `drizzle-kit generate`, not introduced by this change). Added `DrizzleTaskRepository.createReceiptChainIntent`/`getLatestPassingEvidence`/`getReceiptChainReconciliationContext`/`confirmReceiptSubmittedFromReconciliation` mirroring the `create_task` intent lifecycle but authorized to the task's assignee wallet, requiring chain-confirmed `open` state and a passing evidence bundle, and rejecting a second concurrent receipt intent. Widened `ChainTransactionView`/`ChainTransactionRecord`'s `intentType` from a hardcoded `"create_task"` literal to the full already-frozen union (`chainView`/`parseChainSnapshot` were quietly discarding the real persisted value before). Reused the existing intent-agnostic `recordWalletOutcome`/`markTransactionUnknown`/`markTransactionReverted` DB methods as-is for receipts. Added `DoneBondRepository.getPublicReceipt`/`getReceiptForMember` joining task/chain-transaction/receipt-attestation/evidence-bundle/verification-checks. Added a small isolated `receipt-attestation.ts` signing module (`signReceiptAttestation`, `computeIntegrityStatus`) that takes the verifier private key as an explicit parameter (never reads `process.env`) and reuses `computeReceiptAttestationDigest` plus a newly-exported `PASSING_RECEIPT_TYPES` constant from `@donebond/shared` so the signed EIP-712 payload is provably identical to the one the digest function hashes. Added `receipt-reconciliation.ts` mirroring `task-reconciliation.ts` for the `ReceiptSubmitted`/`VerifierAttestationConsumed` events. Added `receipt-handlers.ts` (assignee-only intent creation, wallet-outcome recording, reconciliation, and both public/member reads with an explicit field-allowlist DTO) and `receipt-runtime.ts` (DB-error translation, store wiring, startup validation of `VERIFIER_PRIVATE_KEY`, now also invoked from `validate-startup.ts`). Extended `jsonResponse` with an optional cache-control override (default unchanged at `no-store`) so only the confirmed public receipt response can opt into short-lived public caching.
- Files changed: `.env.example`; `packages/shared/src/domain.ts` (exported `PASSING_RECEIPT_TYPES`); `packages/db/src/schema.ts`, `packages/db/src/repository.ts`, `packages/db/src/task-chain-repository.ts`, `packages/db/migrations/0005_purple_the_twelve.sql` (new), `packages/db/migrations/meta/0005_snapshot.json` (new), `packages/db/migrations/meta/_journal.json`, `packages/db/test/schema.test.mjs`, `packages/db/test/repository.test.mjs`, `packages/db/test/task-chain-repository.test.mjs`; `apps/web/src/server/http.ts`, `apps/web/src/server/task-handlers.ts` (widened `ChainTransactionRecord.intentType`), `apps/web/src/server/project-policy-handlers.ts` (widened `ProjectWriteOperation`), `apps/web/src/server/project-write-rate-limiter.test.ts`, `apps/web/src/server/auth-runtime.ts` (new rate-limit buckets), `apps/web/src/server/validate-startup.ts`, `apps/web/src/server/receipt-attestation.ts` (new), `apps/web/src/server/receipt-attestation.test.ts` (new), `apps/web/src/server/receipt-reconciliation.ts` (new), `apps/web/src/server/receipt-handlers.ts` (new), `apps/web/src/server/receipt-handlers.test.ts` (new), `apps/web/src/server/receipt-runtime.ts` (new), `apps/web/src/server/receipt-runtime.test.ts` (new); five new route files under `apps/web/src/app/api/v1/tasks/[taskId]/receipt-intent/`, `.../receipt-transactions/`, `.../receipt/`, `apps/web/src/app/api/v1/chain/reconcile-receipt/[transactionId]/`, `apps/web/src/app/api/v1/receipt/[receiptId]/`.
- Verification commands: `pnpm format:check && pnpm lint && pnpm typecheck && pnpm test && pnpm test:contracts && pnpm build`; `bash scripts/verify-git-identity.sh`; `node scripts/scan-secrets.mjs`; `git diff --cached --check`.
- Results: All pass. shared 16/16, evidence 36/36, db 74/74 (1 pre-existing guarded Postgres integration test skipped), cli 22/22, web 97/97 (15 new tests: 4 in `receipt-attestation.test.ts` covering digest/signature correctness against `computeReceiptAttestationDigest`/`recoverAddress` and verified/mismatch integrity outcomes, 1 in `receipt-runtime.test.ts` for DB error translation, 10 in `receipt-handlers.test.ts` covering assignee-only authorization against both the creator and an unrelated wallet, non-member/unknown-task indistinguishability, missing-passing-evidence rejection, ineligible task-state rejection, idempotent intent replay, unknown-transaction reconciliation, and the public/member field-allowlist with an explicit expected-key-set assertion plus a verifier-mismatch integrity case), contracts 32/32. Production build registers all five new routes as dynamic Node routes. Secret scanner reports only the one pre-existing, already-accepted seeded-fake-AWS-key finding in `evidence-runtime.test.ts` from milestone 4.6, unrelated to this change; `git diff --check` and the identity script both pass.
...
## 2026-07-19 — Codex — deployed-web verification and Supabase TLS repair

- Live verification: Opened `https://done-bond-web.vercel.app` in a real browser. The landing page and Monad RPC boundary rendered successfully, but `/api/v1/health` returned `503` with `database: false` and `rpc: true`; the page also requested a missing favicon and retained obsolete milestone copy.
- Fix: Corrected the PostgreSQL TLS-mode mapping so `DATABASE_SSL=require` provides Supabase-compatible encrypted transport, added an explicit fail-closed `verify-full` mode requiring `DATABASE_CA_CERT`, retained transaction-pooler-safe prepared-statement settings, and documented both modes. Health failures now emit only allowlisted infrastructure codes in structured logs, never connection strings, queries, or raw error messages.
- Product cleanup: Added the application icon, removed obsolete claims that project creation and public proof were unfinished, and added a browser regression for the implemented project link.
- Verification: Full `pnpm verify` passes (format, lint/boundaries, typecheck, workspace tests, 32 Foundry tests, production build, and 4/4 Playwright E2E checks). Focused results are database 74/75 with only the guarded live-PostgreSQL test skipped and web 104/104. Browser artifacts were removed before commit.
- Remaining deployment step: The current hosted build remains degraded until this commit is redeployed. For immediate compatibility keep `DATABASE_SSL=require`; for strongest verification configure `DATABASE_SSL=verify-full` with Supabase's CA in `DATABASE_CA_CERT`. The previously exposed database password must be rotated before redeployment, then `/api/v1/health` and the wallet-signed end-to-end flow should be rechecked.
